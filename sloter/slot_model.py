...
def load_backbone(args):
    ...
    if args.use_slot:
        if args.use_pre:
            ...
            print("Loaded pre-trained dataset parameters.")
        if not args.grad:
...
class SlotModel(nn.Module):
    def __init__(self, args):
        ...
        if self.use_slot:
            ...
            self.slot = SlotAttention(
                ...
                num_heads=args.num_heads,  # Pass num_heads to SlotAttention
            )
    ...
    def forward(self, x, target=None):
        ...
        if self.use_slot:
            ...
            b, n, r, c = x.shape
            x = x.reshape((b, n, -1)).permute(
                (0, 2, 1)
            )  # Shape: [batch_size, num_patches, hidden_dim]
            x_pe = x_pe.reshape((b, n, -1)).permute(
                (0, 2, 1)
            )  # Shape: [batch_size, num_patches, hidden_dim]
